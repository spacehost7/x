...
STATE_FILE = "state_jp_morning.json"
...
def fetch_tokyo_morning_data():
    """
    NOTE:
      ここではダミーデータを返しています。
      実際には、日経平均・TOPIXを取得できるマーケットデータAPI
      （例: Alpha Vantage, Finnhub, 日本の証券会社APIなど）を使ってください。
      そのAPIキーやシンボルは環境変数経由かコード内コメントで管理します。

    戻り値は、GeminiならぬClaudeに渡しやすいようにシンプルなdictにしています。
    """
    # TODO: 本番ではAPI呼び出しに置き換え
    return {
        "nikkei_close": 39000,
        "nikkei_change": +250,
        "nikkei_change_pct": +0.65,
        "topix_close": 2800,
        "topix_change": +15,
        "topix_change_pct": +0.54,
    }
...
フォロワーに向けて、東京市場の前場の動きを日本語で短く解説します。
...
これらを踏まえて、「投資家カナ」として前場のまとめコメントを1つだけ出力してください。
...
def main():
    ...
    market = fetch_tokyo_morning_data()
